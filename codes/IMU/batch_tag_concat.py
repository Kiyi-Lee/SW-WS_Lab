...
for dirpath, dirnames, filenames in os.walk(root):
    ...
    for filepath in filenames:
        if fnmatch(filepath, '*.csv'):
            ...
            df.insert(loc=df.shape[1], column='type', value=list[filepath.partition('-')[2].partition('-')[0]])

            print("df.shape:{0:>6}".format(str(df.shape)))

            if flag:
                for i in range(0,6):
                    # d1-d3: A-x,y,z , d4-d6: W-x,y,z ,  d7: userId , d8: type
                    ...
                    d7_new = np.append(d7_new,df.iloc[:,df.shape[1]-2:df.shape[1]-1].to_numpy())
                    d8_new = np.append(d8_new,df.iloc[:,df.shape[1]-1:df.shape[1]].to_numpy())
                ...
                d7 = df.iloc[:,df.shape[1]-2:df.shape[1]-1].to_numpy()
                d8 = df.iloc[:,df.shape[1]-1:df.shape[1]].to_numpy()
                ...
                for i in range(1,6):
                    ...
                    d7 = np.append(d7,df.iloc[:,df.shape[1]-2:df.shape[1]-1].to_numpy())
                    d8 = np.append(d8,df.iloc[:,df.shape[1]-1:df.shape[1]].to_numpy())
...
d_new = pd.DataFrame(d_new.reshape((d_new.shape[0]//8), 8, order='F'), columns=['A-x', 'A-y', 'A-z', 'W-x', 'W-y', 'W-z', 'user', 'type'])
