...
for dirpath, dirnames, filenames in os.walk(root):
    for filepath in filenames:
        if fnmatch(filepath, '*.csv'):
            ...
            filepath_list.append(filepath.rstrip('.csv'))
...
for j, path in enumerate(path_list):
    ...
    # caculate fs
    # df.timeStampInMill.max() - df.timeStampInMill.min()
    d1 = datetime.datetime.strptime(df.timeStampInMill.max(), '%H:%M:%S:%f')
    d2 = datetime.datetime.strptime(df.timeStampInMill.min(), '%H:%M:%S:%f')

    seconds = (d1 - d2).seconds + (d1 - d2).microseconds // 1000 /1000
    fs_insole = round(len(df) / seconds, 2)

    print("开始时间戳：{},结束时间戳：{}".format(d2, d1))
    print("鞋垫采样时间：{}, int(fs_insole):{}, round.2(fs_insole):{}".format(seconds, int(len(df) / seconds), fs_insole))
    ...
    plt.suptitle('Insole Pressure ' + filepath_list[j].replace('.csv', ''), fontsize='10')
    plt.xlabel('Time', fontsize ='10')
    plt.ylabel('Pressure', fontsize='10')
    plt.minorticks_on()
    plt.plot(np.arange(df.shape[0])/fs_insole,df['pressureTwo'].values, color='pink', label='pressureOne')
    plt.plot(np.arange(df.shape[0])/fs_insole,df['pressureThree'].values, color='blue', label='pressureTwo')
    plt.plot(np.arange(df.shape[0])/fs_insole,df['pressureFive'].values, color='lightgreen', label='pressureThree')
    plt.grid(True, which="major", axis='x', linestyle="--", color="gray", linewidth=0.75)
    plt.grid(True, which="major", axis='y', linestyle="--", color="gray", linewidth=0.75)
    plt.xlim(0, df.shape[0]/fs_insole) # 设置x轴的刻度范围
    plt.ylim(0, df['pressureFour'].max()+100) # 设置y轴的刻度范围
